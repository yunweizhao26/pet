...
class WicTaskHelper(TaskHelper):
    ...
    def get_sequence_classifier_inputs(self, example: InputExample) -> Dict[str, Any]:
        ...
        return self.wrapper.tokenizer.encode_plus(
            text_a, example.text_b, add_special_tokens=True, max_length=self.wrapper.config.max_seq_length, truncation=True
        )
...
class MultiRcTaskHelper(TaskHelper):
    ...
    def get_sequence_classifier_inputs(self, example: InputExample) -> Dict[str, Any]:
        ...
        return self.wrapper.tokenizer.encode_plus(
            text_a, text_b, add_special_tokens=True, max_length=self.wrapper.config.max_seq_length, truncation=True
        )
...
class WscTaskHelper(TaskHelper):
    ...
    def add_features_to_dict(self, features: List[InputFeatures], feature_dict: Dict[str, torch.Tensor]) -> None:
        ...
        feature_dict["target_id"] = torch.tensor(
            [len(self.id_to_target) + idx for idx, f in enumerate(features)], dtype=torch.long
        )
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], decoding_strategy: str = "default", **kwargs):
        ...
        assert batch["input_ids"].shape[0] == 1, "eval_step() for WSC is only implemented for batch_size=1"
...
class RecordTaskHelper(TaskHelper):
        ...
        for num_masks, choices_with_labels in choices_grouped_by_length.items():
            ...
            for batch in chunks(choices_with_labels, batch_size):
                ...
                probs = self._get_choice_probabilities_batched(
                    choice_ids, batch_input_ids, initial_outputs[num_masks], decoding_strategy=decoding_strategy
                )
    ...
    def _get_choice_probabilities_batched(self, target_sequences, input_ids, initial_output, decoding_strategy):
        ...
        while True:
            masks = {
                batch_idx: [(idx, tok) for idx, tok in enumerate(target_sequences[batch_idx]) if tok >= 0]
