...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="The output directory where the model predictions and checkpoints will be written. [TASK_NAME]"
    " will be replaced by the task name.",
)
parser.add_argument(
    "--save_model", action="store_true", help="Whether to save model binaries. Keep disa bled to save space."
)
...
parser.add_argument(
    "--pattern_ids", default=[0], type=int, nargs="+", help="The ids of the PVPs to be used (only for PET)"
)
parser.add_argument(
    "--lm_training", action="store_true", help="Whether to use language modeling as auxiliary task (only for PET)"
)
parser.add_argument(
    "--alpha",
    default=0.9999,
...
parser.add_argument(
    "--verbalizer_file", default=None, help="The path to a file to override default verbalizers (only for PET)"
)
parser.add_argument(
    "--reduction",
    default="wmean",
...
parser.add_argument(
    "--no_distillation", action="store_true", help="If set to true, no distillation is performed (only for PET)"
)
parser.add_argument(
    "--pet_repetitions",
    default=3,
...
parser.add_argument(
    "--pet_per_gpu_train_batch_size", default=4, type=int, help="Batch size per GPU/CPU for PET training."
)
parser.add_argument(
    "--pet_per_gpu_eval_batch_size", default=8, type=int, help="Batch size per GPU/CPU for PET evaluation."
)
parser.add_argument(
...
parser.add_argument(
    "--pet_num_train_epochs", default=3, type=float, help="Total number of training epochs to perform in PET."
)
parser.add_argument(
    "--pet_max_steps",
    default=-1,
...
parser.add_argument(
    "--pet_min_steps", default=-1, type=int, help="If > 0: set minimal number of steps. Use with num_train_epochs."
)
...
parser.add_argument(
    "--sc_min_steps", default=-1, type=int, help="If > 0: set minimal number of steps. Use with num_train_epochs."
)

# iPET-specific optional parameters
parser.add_argument(
    "--ipet_generations", default=3, type=int, help="The number of generations to train (only for iPET)"
)
parser.add_argument(
...
parser.add_argument(
    "--cache_dir", default="", type=str, help="Where to store the pre-trained models downloaded from S3."
)
