...
def main(logger, args):
    ...
    result_path = os.path.join(args.out_dir, "results.txt")
    # row_list = [["Task", "Dataset", "k", "Model", "Prompt ID", "Seed", "Method", "Accuracy"]]
    row_list = []
    for seed in seeds:
        ### data ...
        ...
        for test_task in tqdm(dev_counter):
            ...
            row_list.append([test_task, args.task_type, args.k, args.gpt2, args.tmp_id, seed, args.method, result])
            if result is None:
    ...
    with open('studentsq.csv', 'a') as file:
...
def run(logger, task, metaicl_data, metaicl_model, train_data, dev_data, seed,
    ...
    if args.do_zeroshot:
        ...
        cache_path = os.path.join(args.out_dir,
                                  "{}-{}-{}{}{}{}{}.pkl".format(
                                      task,  # e.g. glue-rte a.k.a dataset
                                      split_name,  # e.g.test
                                      metaicl_data.method,  # direct or channel
...
if __name__ == '__main__':
    ...
    parser.add_argument("--tmp_id", type=int, default=0)
    ...
    logging.basicConfig(filename=os.path.join(args.out_dir, "logs.txt"),
                        filemode='a',
