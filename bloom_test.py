...
def main(logger, args):
    ...
    metaicl_model = BloomModel(args.bloom, logger)
    ...
    logger.info(
        "Macro-F1 of %s over %d target tasks: %.1f" % (args.task, len(results) // len(seeds), 100 * np.mean(results)))
...
def run(logger, task, metaicl_data, metaicl_model, train_data, dev_data, seed,
    ...
    if args.do_zeroshot:
        ...
        bloom_cache_path = os.path.join(args.out_dir,
                                       "{}-{}-{}{}{}{}.json".format(
                                           task,
                                           split_name,
                                           metaicl_data.method,
        ...
        bloom_cache_path = os.path.join(args.out_dir, "{}-{}-{}{}{}.json".format(
            task,
            args.split,
            metaicl_data.method,
            "-k={}".format(args.k) if args.use_demonstrations else "",
    ...
    bloom_dataloader, bloom_metadata = metaicl_model.prepare_data(train_data if args.use_demonstrations else [],
    ...
    if os.path.exists(cache_path):
        ...
        losses, bloomcache = metaicl_model.do_inference(bloom_dataloader)
        with open(cache_path, "wb") as f:
            pkl.dump(losses, f)
        with open(bloom_cache_path, "w") as f:
            json.dump(bloomcache, f)
    ...
    predictions = metaicl_model.do_predict(losses=losses, metadata=bloom_metadata)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--bloom", type=str, default="bloom")
